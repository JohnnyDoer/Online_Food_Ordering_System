...
class CustomerPageTests(TestCase):
    ...
    def test_customer_signup(self):
        c = Client()
        response = c.post('/customer/signup/', {'username': 'save12', 'email': 'eede@gmail.com', 'password': 'asdfghjkl12'})
        self.assertEquals(response.status_code,200)

    def test_customer_login(self):
        c = Client()
        response = c.post('/customer/login/', {'username': 'save12', 'password': 'asdfghjkl12'})
        self.assertEquals(response.status_code,200)
...
class ModelsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        user = User.objects.create(username="testuser", email="testuser@gmail.com", password="Hello World")
        Food_Res_ID = Restaurant.objects.create(user=user, Restaurant_Name="Bawarchi", Restaurant_Street="fbxfb" , Restaurant_Phone_Number="+919440596242")
        Food_Category_ID = FoodCategory.objects.create(FoodCategory_Name="Drinks")
        Item_Food_ID = Food.objects.create(Food_Name="Milkshake", Food_Price="120", Food_Res_ID=Food_Res_ID, Food_Category_ID=Food_Category_ID)
        Cart_Customer_ID = Profile.objects.create(user=user, Customer_First_Name="Thota", Customer_Last_Name="Chow", Customer_Phone_Number="+919440596242")
        Order_Address = Address.objects.create(Customer_ID=Cart_Customer_ID, Home="Saddad11", Street="1234", city=city, area=area)
        Item_Order_ID = Order.objects.create(Order_Customer_ID=Cart_Customer_ID, Order_Address=Order_Address,Order_Restaurant_ID=Food_Res_ID)
        Item.objects.create(Item_Order_ID=Item_Order_ID, Item_Food_ID=Item_Food_ID, Item_Quantity=3, Item_Price=120)

    def test_items_user(self):
        testuser=Item.objects.get(Item_Price=120)
        self.assertEqual(testuser.Item_Order_ID.Order_Customer_ID.user.username, "testuser")
